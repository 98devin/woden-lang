...
export mega-fseq = (evaluate, block, ast) ->
    manualpush arity(0) (stack) !->
        evaluate(block, stack, ast) # that oughta do it
...
export runes = {
    ...
    conditional1: (b, c, evaluate, ast) ->
        manualpush arity(1) (stack) !->
            a = stack.pop!
            if truthy a
                push evaluate(b, stack, ast), stack
            else
                push evaluate(c, stack, ast), stack
}
...
export id = manualpush arity(1) (stack) !->
            ...
            for transform in @transforms
                ...
                case \filter
                    unless truthy (push transform.func, ^^item)[*-1]
                        @base-pos++
                        return false if (times-tried++ > give-up-after) # notify failure
                        continue main
                    times-tried = 0
                case \map
